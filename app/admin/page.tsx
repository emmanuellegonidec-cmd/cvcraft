...
async function getStats() {
  ...
  // Utilisateurs actifs (avec au moins 1 offre)
  const { data: activeUsersData } = await adminClient
    .from('jobs')
    .select('user_id')

  const activeUsers = new Set(activeUsersData?.map((j: { user_id: string }) => j.user_id) ?? []).size
...
export default async function AdminDashboard() {
  ...
  const statCards = [
    ...
    },
    {
      ...
      sub: `+${stats.newJobsThisMonth} ce mois`,
      color: '#E8151B',
      icon: '💼',
    },
    {
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 mb-8 text-sm font-medium">
        Vue d'ensemble de Jean Find My Job
      </p>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <span
                ...
                style={{ backgroundColor: card.color, color: '#111' }}
              >
                ↑
              </span>
            </div>
