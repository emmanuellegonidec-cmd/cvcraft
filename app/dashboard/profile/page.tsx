  ...
  return (
    <div className={montserrat.className} style={{ minHeight: '100vh', background: '#fafafa' }}>
      ...
      <div style={{ background: '#111', color: '#fff', padding: '0 32px', display: 'flex', alignItems: 'center', gap: 16, height: 64, borderBottom: '3px solid #F5C400', position: 'sticky', top: 0, zIndex: 100 }}>
        <button onClick={() => router.push('/dashboard')} style={{ background: 'none', border: '2px solid #444', color: '#fff', borderRadius: 6, padding: '6px 14px', cursor: 'pointer', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", fontWeight: 700, fontSize: 13 }}>
          ← Retour
        </button>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '40px 24px 80px' }}>
          ...
          <Field label="Types de contrat recherchés">
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 4 }}>
              {CONTRACT_OPTIONS.map((ct) => {
                ...
                return (
                  <button key={ct} onClick={() => toggleContract(ct)} style={{ padding: '7px 14px', border: `2px solid ${active ? '#111' : '#ccc'}`, borderRadius: 6, background: active ? '#111' : '#fff', color: active ? '#F5C400' : '#555', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", fontWeight: 700, fontSize: 13, cursor: 'pointer', boxShadow: active ? '2px 2px 0 #F5C400' : 'none', transition: 'all 0.15s' }}>
                    {ct}
                  </button>
                );
              })}
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 32 }}>
          <button onClick={handleSave} disabled={saving} style={{ background: saved ? '#1A7A4A' : '#F5C400', color: saved ? '#fff' : '#111', border: '2px solid #111', borderRadius: 8, padding: '12px 32px', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", fontWeight: 900, fontSize: 15, cursor: saving ? 'not-allowed' : 'pointer', boxShadow: '3px 3px 0 #111', transition: 'all 0.15s' }}>
        ...
        <div style={{ marginTop: 48, borderTop: '2px dashed #E8151B', paddingTop: 32 }}>
          ...
          <button onClick={() => setShowDeleteModal(true)} style={{ background: '#fff', color: '#E8151B', border: '2px solid #E8151B', borderRadius: 8, padding: '10px 24px', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", fontWeight: 700, fontSize: 14, cursor: 'pointer', boxShadow: '2px 2px 0 #E8151B' }}>
      ...
      {showDeleteModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 24 }}>
          <div style={{ background: '#fff', border: '3px solid #E8151B', borderRadius: 12, boxShadow: '5px 5px 0 #E8151B', maxWidth: 480, width: '100%', padding: 32, fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end' }}>
              <button onClick={() => { setShowDeleteModal(false); setDeleteConfirmText(''); }} style={{ padding: '10px 20px', border: '2px solid #111', borderRadius: 6, background: '#fff', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", fontWeight: 700, cursor: 'pointer', fontSize: 14 }}>Annuler</button>
              <button onClick={handleDeleteAccount} disabled={deleteConfirmText !== 'SUPPRIMER' || deleting} style={{ padding: '10px 20px', border: '2px solid #E8151B', borderRadius: 6, background: deleteConfirmText === 'SUPPRIMER' ? '#E8151B' : '#eee', color: deleteConfirmText === 'SUPPRIMER' ? '#fff' : '#aaa', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", fontWeight: 900, cursor: deleteConfirmText === 'SUPPRIMER' ? 'pointer' : 'not-allowed', fontSize: 14 }}>
