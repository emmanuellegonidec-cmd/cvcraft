...
export default function ActionsSection({
  ...
  return (
    <div style={{ padding: 16 }}>
      {loading ? (
        <p style={{ fontSize: 13, color: '#888', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>Chargement...</p>
      ) : isEmpty ? (
        <p style={{ fontSize: 13, color: '#888', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", textAlign: 'center', padding: '20px 0' }}>
          {compact && '✅ '}{emptyText}
        </p>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {upcoming.length > 0 && (
            <>
              <p style={{ fontSize: 10, fontWeight: 700, color: '#888', textTransform: 'uppercase', letterSpacing: '0.5px', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", marginBottom: 4 }}>À venir</p>
              {upcoming.map(action => (
                <ActionCard
                  key={action.id}
                  action={action}
          ...
          {past.length > 0 && (
            <>
              <p style={{ fontSize: 10, fontWeight: 700, color: '#aaa', textTransform: 'uppercase', letterSpacing: '0.5px', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", marginTop: 8, marginBottom: 4 }}>
      ...
      {deleteTarget && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 20px' }}>
          <div style={{ background: '#fff', border: '2px solid #E8151B', borderRadius: 12, boxShadow: '4px 4px 0 #E8151B', padding: 28, maxWidth: 420, width: '100%', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setDeleteTarget(null)} style={{ flex: 1, padding: '10px', fontSize: 13, fontWeight: 700, background: '#fff', border: '2px solid #ccc', borderRadius: 8, cursor: 'pointer', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>Annuler</button>
              <button onClick={() => handleDelete(deleteTarget.id)} style={{ flex: 1, padding: '10px', fontSize: 13, fontWeight: 800, background: '#E8151B', color: '#fff', border: '2px solid #E8151B', borderRadius: 8, cursor: 'pointer', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>Supprimer</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div style={{
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
          <span style={{ fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", fontWeight: 700, fontSize: 13, color: '#111' }}>
            {action.nom}
          </span>
          {action.categorie && (
        ...
        <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          {action.organisateur && (
            <span style={{ fontSize: 11, color: '#555', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>📍 {action.organisateur}</span>
          )}
          <span style={{ fontSize: 11, color: '#555', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>
        ...
        {linkedContacts.length > 0 && (
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center', marginTop: 6 }}>
            <span style={{ fontSize: 11, color: '#888', marginRight: 2, fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>👥</span>
            {linkedContacts.map((c) => (
              ...
              >
                ...
                <span style={{ fontSize: 11, fontWeight: 600, color: '#111', fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>
                  {c.name}
                </span>
              </span>
            ))}
        ...
        {action.note && (
          <p style={{ fontSize: 11, color: '#777', marginTop: 4, fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif" }}>{action.note}</p>
        )}
      </div>
      <div style={{ display: 'flex', gap: 6 }}>
        <button onClick={onEdit} style={{ background: 'none', border: '1.5px solid #111', padding: '4px 10px', cursor: 'pointer', fontSize: 11, fontFamily: "var(--font-montserrat), 'Montserrat', sans-serif", fontWeight: 600 }}>✏️</button>
