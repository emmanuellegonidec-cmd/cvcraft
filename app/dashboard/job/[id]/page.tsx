...
export default function JobPage() {
  ...
  async function addCustomStage() {
    ...
    const { data } = await supabase.from('pipeline_stages').insert({
      user_id: userId, label: newStageName.trim(), color: newStageColor, position: maxPos + 1,
    }).select().single();
    if (data) {
      ...
      <div style={{ padding: '1.5rem', maxWidth: 1100, margin: '0 auto' }}>
        ...
        <div style={{ background: '#fff', border: '2px solid #111', borderRadius: 12, padding: '1.25rem', marginBottom: '1.25rem', boxShadow: '3px 3px 0 #111' }}>
            ...
            {detailStages.map((s, i) => {
              ...
              return (
                <div key={s.id} style={{ position: 'relative', flex: '1 1 0', minWidth: 80, zIndex: isActive ? 2 : 1 }}>
                  <button onClick={() => handleStageChange(s.id)} style={{ width: '100%', background: isActive ? s.color : isPast ? s.color + '33' : '#F4F4F4', color: isActive ? '#fff' : isPast ? s.color : '#888', border: `2px solid ${isActive ? s.color : isPast ? s.color + '66' : '#E0E0E0'}`, borderRadius: i === 0 ? '8px 0 0 8px' : i === detailStages.length - 1 ? '0 8px 8px 0' : '0', marginLeft: i === 0 ? 0 : -2, padding: '8px 6px', fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'Montserrat,sans-serif', whiteSpace: 'nowrap', transition: 'all 0.15s', textAlign: 'center' }}>
                    {s.label}
                    {isCustom && <span onClick={e => { e.stopPropagation(); deleteCustomStage(s.id); }} style={{ marginLeft: 4, opacity: 0.5, cursor: 'pointer', fontSize: 9 }}>✕</span>}
                  </button>
                </div>
              );
            })}
          ...
          {showAddStage && (
            <div style={{ marginTop: 12, background: '#F4F4F4', border: '1.5px solid #E0E0E0', borderRadius: 8, padding: 12, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <input value={newStageName} onChange={e => setNewStageName(e.target.value)} placeholder="Nom de l'étape..." className="fi" style={{ flex: 1, minWidth: 150 }} onKeyDown={e => e.key === 'Enter' && addCustomStage()} autoFocus />
              <input type="color" value={newStageColor} onChange={e => setNewStageColor(e.target.value)} style={{ width: 36, height: 36, border: '2px solid #E0E0E0', borderRadius: 6, cursor: 'pointer', padding: 2 }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isInterviewStage(job.status, customStages) ? '1fr 1fr' : '1fr', gap: '1.25rem', marginBottom: '1.25rem' }}>
          <div style={{ background: '#fff', border: '2px solid #111', borderRadius: 12, padding: '1.25rem', boxShadow: '3px 3px 0 #111' }}>
            ...
            <textarea defaultValue={job.notes || ''} placeholder="Notes, impressions, points clés..." className="fi" style={{ resize: 'vertical', minHeight: 100 }} onBlur={e => handleFieldUpdate('notes', e.target.value)} />
          </div>
          {isInterviewStage(job.status, customStages) && (
            <div style={{ background: '#fff', border: '2px solid #111', borderRadius: 12, padding: '1.25rem', boxShadow: '3px 3px 0 #111' }}>
              ...
              <textarea defaultValue={(job as any).interview_summary || ''} placeholder="Déroulement, questions posées, impressions..." className="fi" style={{ resize: 'vertical', minHeight: 100 }} onBlur={e => handleFieldUpdate('interview_summary', e.target.value)} />
            </div>
          )}
        </div>
