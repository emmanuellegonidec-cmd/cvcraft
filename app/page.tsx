    ...
    <div style={{ fontFamily: "'Montserrat', sans-serif", background: '#FFFFFF', color: '#111111', lineHeight: '1.6' }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-red:active { transform:translate(1px,1px);box-shadow:2px 2px 0 #111; }
        ...
        .btn-outline:hover { background:#F4F4F4;transform:translate(-1px,-1px);box-shadow:5px 5px 0 #E0E0E0; }
        ...
        .step-card { background:rgba(255,255,255,0.08);border:2px solid rgba(255,255,255,0.2);border-radius:12px;padding:1.75rem; }
      ...
      <nav style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'1rem 2.5rem',background:'#fff',borderBottom:'2.5px solid #111',position:'sticky',top:0,zIndex:100 }}>
        <Link href="/" style={{ textDecoration:'none',display:'flex',alignItems:'center',gap:10 }}>
          <div style={{ background:'#E8151B',border:'2.5px solid #111',borderRadius:8,padding:'4px 12px',boxShadow:'2px 2px 0 #111' }}>
            <span style={{ fontFamily:'Montserrat,sans-serif',fontSize:'1.1rem',fontWeight:900,color:'#F5C400',letterSpacing:'-0.02em' }}>Jean</span>
          </div>
          <span style={{ fontFamily:'Montserrat,sans-serif',fontSize:'0.85rem',fontWeight:700,color:'#111',letterSpacing:'0.05em',textTransform:'uppercase' }}>Find My Job</span>
        </Link>
      ...
      <section style={{ background:'#fff',borderBottom:'2.5px solid #111',overflow:'hidden' }}>
        <div className="hero-grid" style={{ maxWidth:1100,margin:'0 auto',padding:'5rem 2rem 4rem',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'4rem',alignItems:'center' }}>
          <div>
            <div className="fade1" style={{ display:'inline-flex',alignItems:'center',gap:6,background:'#F5C400',border:'2px solid #111',borderRadius:20,padding:'5px 14px',fontSize:12,fontWeight:800,color:'#111',marginBottom:'1.5rem',boxShadow:'2px 2px 0 #111',textTransform:'uppercase',letterSpacing:'0.05em' }}>
          ...
          <div id="dashboard" style={{ background:'#fff',borderRadius:12,border:'2.5px solid #111',boxShadow:'6px 6px 0 #111',overflow:'hidden' }}>
            ...
            <div style={{ display:'grid',gridTemplateColumns:'150px 1fr' }}>
              <div style={{ background:'#F4F4F4',borderRight:'2px solid #E0E0E0',padding:'14px 10px' }}>
                <div style={{ fontSize:10,fontWeight:800,color:'#888',marginBottom:10,letterSpacing:'0.08em',textTransform:'uppercase' }}>Jean</div>
      ...
      {/* STATS */}
      <div className="stats-row" style={{ background:'#E8151B',borderBottom:'2.5px solid #111',padding:'2rem 3rem',display:'flex',justifyContent:'center',gap:'6rem',flexWrap:'wrap' }}>
        {[['2 000+','Candidats actifs'],['3x',"Plus d'entretiens"],['30s','Pour un CV'],['4.9★','Note moyenne']].map(([n,l]) => (
          <div key={l} style={{ textAlign:'center' }}>
            ...
            <div style={{ fontSize:13,color:'rgba(255,255,255,0.85)',marginTop:2,fontWeight:600 }}>{l}</div>
          </div>
        ))}
      </div>
      ...
      <section className="feat-grid" style={{ maxWidth:1100,margin:'0 auto',padding:'5rem 2rem',display:'grid',gridTemplateColumns:'1fr 1fr',gap:'5rem',alignItems:'center' }}>
        <div>
          ...
          <p style={{ fontSize:15,color:'#555',lineHeight:1.75,marginBottom:'1.5rem',fontWeight:500 }}>Fini les tableurs Excel. Jean centralise toutes vos candidatures dans un tableau Kanban visuel. Glissez, déposez, suivez en temps réel.</p>
      ...
      <section style={{ background:'#111',borderTop:'2.5px solid #111',borderBottom:'2.5px solid #111',padding:'5rem 2rem' }}>
        <div style={{ maxWidth:900,margin:'0 auto',textAlign:'center' }}>
          <div style={{ display:'inline-block',background:'#F5C400',border:'2px solid rgba(255,255,255,0.3)',borderRadius:20,padding:'5px 16px',fontSize:12,fontWeight:800,color:'#111',marginBottom:'1rem',textTransform:'uppercase',letterSpacing:'0.05em' }}>Simple comme bonjour</div>
          ...
          <div className="steps-grid" style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'2rem',textAlign:'left' }}>
            {[
              { icon:'👤',num:'01',title:'Créez votre profil',desc:'Inscrivez-vous gratuitement et importez votre profil LinkedIn en PDF. Jean extrait automatiquement toutes vos informations.' },
              ...
              <div key={s.num} className="step-card">
                <div style={{ width:44,height:44,borderRadius:10,background:'#E8151B',border:'2px solid rgba(255,255,255,0.3)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,marginBottom:'1rem' }}>{s.icon}</div>
      ...
      <section style={{ padding:'5rem 2rem',background:'#fff',borderBottom:'2.5px solid #111' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom:'3rem' }}>
            <h2 style={{ fontSize:'2.2rem',fontWeight:900,letterSpacing:'-0.02em' }}>Ils ont trouvé leur job avec Jean</h2>
          </div>
          <div className="testi-grid" style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1.5rem' }}>
            {[
              ...
              { initials:'AC',name:'Amandine C.',role:'Reconversion → Talent Acquisition',text:"Jean m'a aidée à valoriser mes compétences transférables. Résultat : CDI en 6 semaines.",color:'#EBF2FD',textColor:'#1A6FDB' },
            ].map(t => (
      ...
      {/* CTA */}
      <section style={{ background:'#E8151B',borderBottom:'2.5px solid #111',padding:'6rem 2rem',textAlign:'center' }}>
        <div style={{ display:'inline-block',background:'#F5C400',border:'2px solid #111',borderRadius:20,padding:'5px 16px',fontSize:12,fontWeight:800,color:'#111',marginBottom:'1.5rem',boxShadow:'2px 2px 0 #111',textTransform:'uppercase',letterSpacing:'0.05em' }}>C&apos;est gratuit !</div>
        <h2 style={{ fontSize:'2.8rem',color:'#fff',marginBottom:'1rem',fontWeight:900,letterSpacing:'-0.02em',textShadow:'3px 3px 0 rgba(0,0,0,0.2)' }}>Prête à trouver votre prochain poste ?</h2>
        <p style={{ color:'rgba(255,255,255,0.85)',fontSize:'1.05rem',marginBottom:'2.5rem',fontWeight:500 }}>Rejoignez 2 000+ candidats qui ont décroché plus d&apos;entretiens avec Jean.</p>
        <Link href="/auth/signup" className="btn-black" style={{ fontSize:16,padding:'16px 44px',borderRadius:12 }}>
          Commencer gratuitement — sans carte bancaire
        </Link>
        <p style={{ marginTop:'1.25rem',fontSize:13,color:'rgba(255,255,255,0.7)',fontWeight:600 }}>Résultat en moins de 30 secondes · Données sécurisées</p>
      </section>

      {/* FOOTER */}
      <footer className="footer-grid" style={{ background:'#111',borderTop:'2.5px solid #111',padding:'3rem',display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',gap:'3rem' }}>
        <div>
          <div style={{ display:'flex',alignItems:'center',gap:10,marginBottom:'0.75rem' }}>
            <div style={{ background:'#E8151B',border:'2px solid rgba(255,255,255,0.3)',borderRadius:8,padding:'4px 12px' }}>
              <span style={{ fontFamily:'Montserrat,sans-serif',fontSize:'1.1rem',fontWeight:900,color:'#F5C400' }}>Jean</span>
            </div>
            <span style={{ fontSize:'0.8rem',fontWeight:700,color:'rgba(255,255,255,0.6)',textTransform:'uppercase',letterSpacing:'0.05em' }}>Find My Job</span>
          </div>
          <p style={{ fontSize:13,color:'rgba(255,255,255,0.45)',lineHeight:1.7,fontWeight:500 }}>La plateforme de recherche d&apos;emploi propulsée par Claude AI. Tableau de bord, CV Creator, suivi des candidatures.</p>
        </div>
        {[
          ...
          <div key={col.title}>
            ...
            {col.links.map(link => (
              <a key={link} href="#" style={{ display:'block',fontSize:13,color:'rgba(255,255,255,0.55)',textDecoration:'none',marginBottom:8,fontWeight:500 }}>{link}</a>
            ))}
          </div>
        ))}
      </footer>
      <div style={{ background:'#111',padding:'1.25rem 3rem',borderTop:'1px solid rgba(255,255,255,0.1)',display:'flex',justifyContent:'space-between' }}>
        <p style={{ fontSize:12,color:'rgba(255,255,255,0.3)',fontWeight:500 }}>© 2026 Jean — Find My Job · Propulsé par Claude AI</p>
        <p style={{ fontSize:12,color:'rgba(255,255,255,0.3)',fontWeight:500 }}>Fait avec ♥ pour les candidats</p>
      </div>
    </div>
  );
}
