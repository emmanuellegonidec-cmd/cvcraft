...
// ============================================================================
// RE-SCORE ATS — Analyse de progression du CV optimisé (parcours "Optimiser ce CV")
// ----------------------------------------------------------------------------
// Différences avec /api/jobs/[id]/ats (l'analyse principale) :
//   - Le CV analysé est FOURNI par l'appelant (pdfBase64 du CV optimisé), au lieu
//     d'être lu depuis job.cv_url.
//   - AUCUNE persistance : on n'écrit PAS dans ats_result (le score de départ est
//     conservé pour la comparaison).
//   - AUCUN décompte : ne consomme pas un des 3 crédits d'analyse.
//   - Renvoie uniquement le nouveau score (et le détail par dimension).
//
// ⚠️ IMPORTANT : la grille de notation ci-dessous (ATS_SYSTEM_PROMPT) DOIT rester
// IDENTIQUE à celle de app/api/jobs/[id]/ats/route.ts, sinon la comparaison
// "ancien score → nouveau score" serait faussée. Toute modification de la grille
...
// ============================================================================
// AUTH HELPER (identique à l'analyse principale)
// ============================================================================
...
// ============================================================================
// POST /api/jobs/[id]/ats-rescore
// Body attendu : { pdfBase64: string }  (le CV optimisé, PDF encodé en base64)
// ============================================================================
export async function POST(
  ...
  try {
    ...
    // Récupération de l'offre (poste + description) pour comparer le CV contre elle.
    ...
    // Le CV optimisé est fourni par l'appelant (PDF base64), pas lu depuis le storage.
    const body = await request.json().catch(() => null)
    const pdfBase64: string | undefined = body?.pdfBase64
    if (!pdfBase64 || typeof pdfBase64 !== 'string') {
      return NextResponse.json(
        { error: 'CV manquant : pdfBase64 requis.' },
        { status: 400 }
      )
    }
...
Rappel : compte bien la couverture keywords (étape 2) avant de scorer la dimension matching.`
    ...
    try {
      ...
      console.error('Parse JSON échoué (re-score), texte brut:', rawText)
      return NextResponse.json(
    ...
    // Recalcul du score_global côté serveur (source de vérité = somme des dimensions).
    ...
    // Pas d'écriture en base, pas de décompte : simple renvoi du nouveau score.
    return NextResponse.json({
      success: true,
      score_global: scoreGlobal,
      scores: atsResult.scores || {},
      recommandations: atsResult.recommandations || [],
    })
  } catch (err: any) {
    console.error('Erreur API ATS re-score:', err)
