...
// ============================================================================
// SYSTEM PROMPT — IDENTIQUE à /api/jobs/[id]/ats/route.ts
// (Synchroniser les 2 routes si la grille évolue. TODO session 9 : factoriser
//  dans lib/ats-prompt.ts pour éviter la duplication.)
// ============================================================================
...
// ============================================================================
// POST /api/jobs/[id]/ats-from-extension
// Body : { cvRef: string }  — ex: "upload:843d.../9884.../cv.pdf"
// ============================================================================
export async function POST(
  ...
  try {
    ...
    // CV Creator non supportés en session 5 (chantier PDF serveur à part)
    if (cvRef.startsWith('creator:')) {
      return NextResponse.json(
        {
          error: "Les CV Creator ne sont pas encore analysables depuis l'extension. Télécharge-le depuis Mes CV puis upload-le sur cette offre.",
          code: 'CREATOR_NOT_SUPPORTED',
        },
        { status: 400, headers: corsHeaders }
      )
    }
    ...
    // Sécurité : le path DOIT commencer par user.id/
    if (!filePath.startsWith(user.id + '/')) {
      return NextResponse.json(
        { error: 'Accès interdit à ce CV' },
    ...
    const { data: job, error: jobError } = await supabase
      .from('jobs')
      .select('id, title, company, description, user_id, ats_analysis_count')
      .eq('id', jobId)
      .eq('user_id', user.id)
      .single()
    ...
    if (downloadError || !fileData) {
      console.error('Erreur download CV:', downloadError)
      return NextResponse.json(
        { error: 'Impossible de télécharger le CV depuis le storage' },
    ...
    const nomFichier = extractFilenameFromPath(filePath)
...
- Chaque point doit être SPÉCIFIQUE au CV analysé (pas de généralités)`
    ...
    // Enrichissement serveur : analyse_fichier
    ...
    // Recalcul du score_global côté serveur (par sécurité)
    ...
    // Sauvegarde dans la table jobs (visible aussi côté web Jean)
    ...
    console.error('Erreur API ATS extension:', err)
    return NextResponse.json(
