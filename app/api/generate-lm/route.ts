...
// ──────────────────────────────────────────────────────────────────
// Auth : Bearer token + cookies fallback (pattern projet)
// ──────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────
// Reconstruit un markdown lisible à partir du form_data d'un CV creator
// (mêmes champs que CVFormData, on ne dépend pas du module CV pour rester léger)
// ──────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────
// Charge le CV de référence selon la ref ("creator:{uuid}" ou "upload:{path}")
// Retourne :
//   - { type: 'markdown', text } pour un CV creator (markdown reconstruit)
//   - { type: 'pdf', base64 } pour un CV upload (PDF en base64 pour Claude Vision)
//   - null si aucun CV trouvé
// ──────────────────────────────────────────────────────────────────
async function loadCvByRef(
  ...
  if (ref.startsWith('creator:')) {
    ...
    // Priorité au form_data (structuré), fallback content (markdown stocké)
  ...
  if (ref.startsWith('upload:')) {
    ...
    // Sécurité : le path doit appartenir à l'utilisateur
...
// ──────────────────────────────────────────────────────────────────
// Validation des options reçues (defensive)
// ──────────────────────────────────────────────────────────────────
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── 1. Charger l'offre (en vérifiant l'appartenance user) ──
    ...
    // ── 3. Résoudre la ref CV : explicite > défaut profil > rien ──
    ...
    // Si CV markdown : on l'inclut directement dans le prompt texte.
    // Si CV PDF : on construit le prompt SANS markdown, et on attache le PDF en doc séparé.
    ...
    // ── 5. Construire le message Claude (text + éventuel PDF attaché) ──
    ...
    // Normalisation : retirer espaces parasites en début/fin
