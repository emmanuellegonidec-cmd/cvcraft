...
// Construit un titre lisible pour une LM uploadée
// Ex: "LM - Chargée de projet web — Maison Lenôtre"
// Fallback si l'offre a été supprimée : "LM - Offre supprimée"
function buildUploadTitle(fileName: string, jobTitle?: string, jobCompany?: string): string {
  const prefix = fileName.toLowerCase().startsWith('lm') ? 'LM' : fileName;
...
export async function GET(req: NextRequest) {
  ...
  // ─────────────────────────────────────────────────────────────
  // MODE "SINGLE LM" — si ?id=xxx est fourni, renvoie la ligne
  // complète de la table lms (title, template, content, form_data).
  // Utilisé par l'éditeur (futur) pour charger une LM existante,
  // et par la page Mes LM pour générer le PDF à la volée.
  // ─────────────────────────────────────────────────────────────
  if (singleId) {
    try {
      ...
      if (error) {
        console.error('GET /api/lms?id=... error:', error);
        return NextResponse.json({ error: error.message }, { status: 500 });
      }

      if (!data) {
        return NextResponse.json({ error: 'LM introuvable' }, { status: 404 });
      }

      return NextResponse.json({ lm: data });
    } catch (err: any) {
      console.error('GET /api/lms?id=... exception:', err);
      return NextResponse.json({ error: err.message || 'Erreur serveur' }, { status: 500 });
    }
  }

  // ─────────────────────────────────────────────────────────────
  // MODE "LISTE AGRÉGÉE" — comportement historique (clone CVs).
  // Retourne toutes les LM (creator + uploads) avec leurs refs.
  // ─────────────────────────────────────────────────────────────
  try {
    const { data: profile } = await supabase
      .from('user_profiles')
      .select('default_lm_ref, lm_display_names')
      .eq('user_id', user.id)
      .maybeSingle();

    const defaultRef: string | null = profile?.default_lm_ref || null;
    const displayNames: Record<string, string> = profile?.lm_display_names || {};

    // LM générées (table lms)
    const { data: creatorLms } = await supabase
      .from('lms')
      .select('id, title, template, created_at, updated_at')
      .eq('user_id', user.id)
      .order('updated_at', { ascending: false });

    const creatorList = (creatorLms || []).map((lm: any) => {
      const ref = `creator:${lm.id}`;
      return {
        ref,
        source: 'creator' as const,
        title: lm.title || 'Sans titre',
    ...
    // LM uploadées (bucket job-documents)
    ...
    if (folders && folders.length > 0) {
      ...
      for (const folder of folders) {
        ...
        for (const file of files) {
          ...
          if (!fname.startsWith('lm')) continue;
    ...
    const allLms = [...creatorList, ...uploadList].sort((a, b) => {
      const ad = new Date(a.updated_at || a.created_at).getTime();
      const bd = new Date(b.updated_at || b.created_at).getTime();
      return bd - ad;
    });

    return NextResponse.json({ lms: allLms, default_ref: defaultRef });
  } catch (err: any) {
    console.error('GET /api/lms error:', err);
    return NextResponse.json({ error: err.message || 'Erreur serveur' }, { status: 500 });
  }
}

// POST — création ou mise à jour d'une LM générée
...
// Renvoie { lm: { id, title, template, created_at, updated_at } }
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (id) {
      ...
      if (error) {
        console.error('POST /api/lms update error:', error);
        return NextResponse.json({ error: error.message }, { status: 500 });
      }
    ...
    if (error) {
      console.error('POST /api/lms insert error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ lm: data });
  } catch (err: any) {
    console.error('POST /api/lms error:', err);
    return NextResponse.json({ error: err.message || 'Erreur serveur' }, { status: 500 });
  }
}
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    if (display_name !== undefined) {
      const currentNames = { ...(profile?.lm_display_names || {}) };
      ...
      updates.lm_display_names = currentNames;
    }

    if (is_default === true) {
      updates.default_lm_ref = ref;
    } else if (is_default === false && profile?.default_lm_ref === ref) {
      updates.default_lm_ref = null;
    }
    ...
    if (error) {
      console.error('PATCH /api/lms error:', error);
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ success: true });
  } catch (err: any) {
    console.error('PATCH /api/lms error:', err);
    return NextResponse.json({ error: err.message || 'Erreur serveur' }, { status: 500 });
  }
}
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    if (ref.startsWith('creator:')) {
      const lmId = ref.slice('creator:'.length);
      const { error } = await supabase
        .from('lms')
        .delete()
        .eq('id', lmId)
    ...
    if (profile?.default_lm_ref === ref) {
      cleanupUpdates.default_lm_ref = null;
    }

    const names = profile?.lm_display_names || {};
    if (names[ref]) {
      const newNames = { ...names };
      delete newNames[ref];
      cleanupUpdates.lm_display_names = newNames;
    }
    ...
    console.error('DELETE /api/lms error:', err);
    return NextResponse.json({ error: err.message || 'Erreur serveur' }, { status: 500 });
  }
}