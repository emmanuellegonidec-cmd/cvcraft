...
export async function generateMetadata(
  ...
  // Fallback : seo_title prime, sinon titre principal + suffixe marque
  const metaTitle = article.seo_title?.trim()
    || `${article.title} — Jean Find My Job`

  const metaDescription = article.seo_description?.trim()
    || article.excerpt?.trim()
    || 'Conseils et ressources pour votre recherche d\'emploi.'

  // Pour Open Graph (réseaux sociaux) : pas de suffixe marque — le siteName est géré à côté
  const ogTitle = article.seo_title?.trim() || article.title
  const ogDescription = article.seo_description?.trim() || article.excerpt?.trim() || ''
