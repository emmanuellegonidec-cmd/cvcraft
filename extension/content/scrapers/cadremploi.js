// extension/content/scrapers/cadremploi.js
// Jean find my Job — Scraper Cadremploi (v1.0)
//
// Strategie : JSON-LD JobPosting primary (couvre titre, entreprise, lieu, salaire,
// description, dates, secteur, type d'emploi) + DOM fallback uniquement pour
// contractType (CDI/CDD/Alternance) quand il n'est pas deductible du JSON-LD.
//
// Specificites Cadremploi :
//   - URL d'offre : https://www.cadremploi.fr/emploi/detail_offre?offreId={ID}
//     (identifiant numerique).
//   - Site Nuxt/Vue avec rendu serveur : le JSON-LD est present dans le HTML.
//   - Deux blocs ld+json coexistent (BreadcrumbList + JobPosting) : on selectionne
//     le noeud @type === "JobPosting".
//   - @context est en http:// : sans incidence, detection via @type.
//   - La description est en texte simple (retours a la ligne \n, pas de balises
//     HTML) : htmlToReadableText la traite via sa branche "texte plain".

(function () {
  'use strict';

  // On ne traite que les pages cadremploi.fr portant un identifiant d'offre (offreId).
  const CADREMPLOI_DOMAIN_REGEX = /^https:\/\/www\.cadremploi\.fr\//i;
  const CADREMPLOI_OFFREID_REGEX = /[?&]offreId=(\d+)/i;
  ...
  // Utile pour CDI/CDD/Alternance que le JSON-LD Cadremploi ne code pas explicitement.
  ...
  function match(url) {
    if (!CADREMPLOI_DOMAIN_REGEX.test(url)) return null;
    const m = url.match(CADREMPLOI_OFFREID_REGEX);
    if (!m) return null;
    return m[1]; // ex: "156542693185994960"
  }
  ...
  function extract(externalId) {
    ...
    // 1. JSON-LD JobPosting : source primaire Cadremploi
  ...
  function extractFromJsonLd() {
    ...
    for (let i = 0; i < scripts.length; i++) {
      try {
        ...
        // Ignore les blocs non-JSON (ex: BreadcrumbList)
      }
    }
    return null;
  }
  ...
  function parseJobPostingLd(j) {
    ...
    // Description + Profil recherché (qualifications) combinés.
    // On concatène les deux pour que le profil idéal soit visible EN ENTIER
    // dans la zone Description du side panel (plutôt que tronqué ailleurs).
    const descText = j.description ? htmlToReadableText(j.description) : '';
    const qualText = j.qualifications ? htmlToReadableText(j.qualifications) : '';

    if (qualText) {
      // Texte complet du profil, conservé aussi à part (analyse / API)
      r.requirements = qualText;
    }

    const descParts = [];
    if (descText) descParts.push(descText);
    if (qualText) descParts.push('Profil recherché\n\n' + qualText);
    if (descParts.length > 0) {
      r.description = descParts.join('\n\n');
    }
  ...
  // Si l'entree ne contient pas de balise (texte simple avec \n), nettoyage direct.
  // Sinon, insertion de sauts de ligne aux balises de bloc puis extraction du texte.
  ...
  window.JFMJ_SCRAPERS.cadremploi = {
    name: 'cadremploi',
    label: 'Cadremploi',
    match: match,
    extract: extract
